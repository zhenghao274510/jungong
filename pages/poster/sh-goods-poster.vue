...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.shareInfo.path = `${bassUrl}/h5/#/pages/goodDetails/gooddetails?id=${options.id}`;
						...
						drawArray: ({
							...
							return new Promise((rs, rj) => {
								rs([
									...
									{
										type: 'image', //商品图片
										url: that.goodsInfo.image || that.sendInfo.image,
										alpha: 1,
										drawDelayTime: 500, //draw延时时间
									...
									},
									{
										type: 'text', //标题
										// fontStyle: 'italic',//倾斜
										text: that.goodsInfo.title || that.sendInfo.title,
										size: fontSize,
										color: 'black',
										alpha: 1,
										textAlign: 'left',
