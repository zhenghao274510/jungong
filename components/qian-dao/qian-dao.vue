<template>
	<!-- 签到 -->
	<view class="pounp-con" @click.stop="hidden" v-if="showPick">
	<view class="qian_dao">
		<view class="header dis_f_sb_c">
			...
			<view class="right dis_f_c_c false" >
				<!-- <text>下月</text> -->
			</view>
		...
		<!-- 	<view class="weeks dis_f_c_c">
			...
			<view class="days dis_f_c_c">
				<view class="ul dis_f_w box">
					...
					<view class="li dis_f_c_c box boleft">
						<text>3积分</text>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			get_this_month_fun(days_list, n, y) {
				...
				let days = new Date().getDate(),leaveDays=0;
				days>6?leaveDays=days-6:leaveDays=1;
				y = this.get_str(y);
				for (var i =leaveDays; i <= days; i++) {
